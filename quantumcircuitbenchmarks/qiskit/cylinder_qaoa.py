...
def generate_cylinder_qaoa_x_y(x, y, seed=0xDEADBEEF):
    i = x*y
...
def generate_cylinder_qaoa_max_size(max_size, seed=0xDEADBEEF):
    x = int(np.floor(np.sqrt(max_size)))
    y = int(np.floor(max_size // x))
    return generate_cylinder_qaoa_x_y(x, y, seed)